...
int main (int argc, char *argv[]) {
  ...
  B = allocDoubleMatrix(ROWS_B, COLUMNS_B);
  ...
  if (rankWorld == MASTER) {
    // allocate an block for row-sized array [references] + actual content [rows*columns]
    A = allocDoubleMatrix(ROWS_A, COLUMNS_A);
    C = allocDoubleMatrix(ROWS_A, COLUMNS_B);
  ...
  for(int i=0; i< ITER ; i++) {
    ...
    /*************************** Worker task ***************************/
      ...
      for (int row=0; row < rows; row++) // compute rows for this thread only
